...
export default function WebGLUnifiedCylinder({ 
  ...
  useEffect(() => {
    ...
    if (trioManager.trios) {
      trioManager.trios.forEach((trio: any) => {
        trio.points.forEach((point: any, pointIndex: number) => {
          const geometry = new THREE.SphereGeometry(0.1, 16, 16) // Very small sphere (0.1cm = 1mm)
    ...
    if (trioManager.currentTrio) {
      trioManager.currentTrio.points.forEach((point: any, pointIndex: number) => {
        const geometry = new THREE.SphereGeometry(0.1, 16, 16) // Very small sphere (0.1cm = 1mm)
        const material = new THREE.MeshBasicMaterial({ 
          color: 0x000000, // Black color
          transparent: false,
  ...
  const handleCanvasClick = useCallback((event: React.MouseEvent<HTMLDivElement>) => {
    ...
    // TEMPORARILY DISABLED: Allow multiple trios without requiring depth for first trio
