...
export default function HollowCylinder({ className = '' }: HollowCylinderProps) {
  ...
  useEffect(() => {
    ...
    console.log('Creating hollow cylinder scene...', { width, height })
    ...
    const innerGeometry = new THREE.CylinderGeometry(innerRadius, innerRadius, cylinderHeight + 0.1, 32)
    ...
    // Create a circular plane for camera feed
    ...
    console.log('Hollow cylinder scene created successfully')
    ...
    // Cleanup function
    return () => {
      console.log('Cleaning up hollow cylinder scene...')
      if (animationId) {
  ...
  return (
    <div className={`hollow-cylinder ${className}`}>
      <div 
        ref={mountRef} 
